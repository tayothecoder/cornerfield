    ...
    public function processTransfer($fromUserId, $toUserId, $amount, $description = '') {
        try {
            ...
            try {
                ...
                // Deduct from sender
                $this->database->query('UPDATE users SET balance = balance - ? WHERE id = ?', [$amount, $fromUserId]);

                // Add to receiver
                $this->database->query('UPDATE users SET balance = balance + ? WHERE id = ?', [$amount, $toUserId]
                );
        ...
        try {
            $result = $this->database->fetchOne(
                "SELECT COALESCE(SUM(amount), 0) as total FROM user_transfers
                 WHERE sender_id = ? AND DATE(created_at) = CURDATE() AND status = 'completed'",
                [$userId]
            );
    ...
    public function cancelTransfer($transferId, $adminId, $reason = '') {
        try {
            ...
            try {
                ...
                // Refund sender
                $this->database->query('UPDATE users SET balance = balance + ? WHERE id = ?', [
